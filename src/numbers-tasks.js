...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const dotSum = x1 * x2 + y1 * y2;
  const denominatorOne = Math.sqrt(x1 ** 2 + y1 ** 2);
  const denominatorTwo = Math.sqrt(x2 ** 2 + y2 ** 2);
  return Math.acos(dotSum / (denominatorOne * denominatorTwo));
}
